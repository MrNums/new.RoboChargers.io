      ...
      <Card className="flex-shrink-0 w-[400px] bg-white rounded-lg shadow-md overflow-hidden transition-all duration-300 hover:shadow-xl border border-gray-100">
        <div className="relative">
          ...
          <div className="absolute top-3 right-3 flex flex-col gap-2">
            ...
            <Badge
              variant="outline"
              className="bg-white/80 backdrop-blur-sm border-[#1a36e8] text-[#1a36e8] font-medium flex items-center justify-center px-3"
            >
              {robot.season}
            </Badge>
          </div>
        ...
        <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold flex items-center">
              <span className="text-[#0a1a70] dark:text-blue-300">
                {robot.name}
              </span>
              <Badge
                variant="outline"
                className="ml-3 border-[#1a36e8] text-[#1a36e8] dark:border-blue-400 dark:text-blue-400"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 my-4">
            <div>
              ...
              {/* Features list */}
              <div className="mt-4">
                <h3 className="font-semibold text-[#0a1a70] dark:text-blue-300 mb-2">
                  Key Features
                </h3>
                      ...
                      <iframe
                        ...
                        src={
                          robot.revealVideoUrl.includes("youtu.be/")
                            ? robot.revealVideoUrl.replace(
                                /youtu\.be\/([^?]+)(\?si=.+)?/,
                                "youtube.com/embed/$1$2",
                              )
                            : robot.revealVideoUrl.replace(
                                /youtube\.com\/watch\?v=([^&]+)(&.+)?/,
                                "youtube.com/embed/$1$2",
          ...
          {robot.awards &&
            robot.awards.some(
              (award) =>
                award.name.includes("Winner") ||
                award.name.includes("Championship"),
            ) && (
              <div className="mt-4 border-t dark:border-zinc-700 pt-4">
                <h3 className="font-semibold text-[#0a1a70] dark:text-blue-300 mb-3">
                  Blue Banners
                </h3>
                <div className="flex overflow-x-auto space-x-2 pb-2">
                  {robot.awards
                    ...
                    .map((award, index) => (
                      <div
                        key={index}
                        className="flex-shrink-0 w-full max-w-[280px] relative"
                      >
                        <LazyImage
